...
if (isset($_POST['signup-submit'])){
    ...
    if (empty($name) || empty($username)  || empty($userpwd)){
        header("Location: signupform.php?error=Empty fields&uname".$username."&name".$name);
        exit();
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)){
            ...
            if($resultCheck > 0){
                header("Location: signupform.php?error=usertaken&name=".$name);
                exit();
            } else { 
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    mysqli_stmt_bind_param($stmt, "sss", $username, $hashedPwd, $name);
                    mysqli_stmt_execute($stmt);
                    mysqli_stmt_store_result($stmt);
                    header("Location: signupform.php?signup=sucess");
                    exit();
                }
            }    
        }
